...
function buildOllama() {
    ...
    & go build -trimpath -ldflags "-s -w -X=github.com/ink-splatters/ollama/version.Version=$script:VERSION -X=github.com/ink-splatters/ollama/server.mode=release" .
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
    if ("${env:KEY_CONTAINER}") {
...
function buildApp() {
    ...
    & go build -trimpath -ldflags "-s -w -H windowsgui -X=github.com/ink-splatters/ollama/version.Version=$script:VERSION -X=github.com/ink-splatters/ollama/server.mode=release" .
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
    if ("${env:KEY_CONTAINER}") {
        & "${script:SignTool}" sign /v /fd sha256 /t http://timestamp.digicert.com /f "${script:OLLAMA_CERT}" `
