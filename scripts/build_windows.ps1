...
function buildOllama() {
    ...
    & go build -trimpath -ldflags "-s -w -X=github.com/ink-splatters/ollama/server.mode=release" .
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
...
function buildApp() {
    ...
    & go build -trimpath -ldflags "-s -w -H windowsgui -X=github.com/ink-splatters/ollama/server.mode=release" -o "${script:SRC_DIR}\dist\windows-${script:TARGET_ARCH}-app.exe" .
    if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
}
